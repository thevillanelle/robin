...
function Chip({ label, value, color = 'var(--c-muted2)' }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
      ...
      <p style={{ ...sans, fontSize: '0.82rem', color, lineHeight: 1, textTransform: 'capitalize' }}>{value}</p>
    </div>
  )
}

function QuizLink({ href, label }) {
  return (
    <a href={href} target="_blank" rel="noreferrer"
      style={{ ...mono, fontSize: '0.56rem', letterSpacing: '0.1em', color: 'var(--c-accent)', textDecoration: 'none' }}>
      {label} ↗
    </a>
  )
}

function EmptyState({ prompt, quizUrl, quizLabel }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      <p style={{ ...sans, fontSize: '0.78rem', color: 'var(--c-dim)', lineHeight: 1.5, fontStyle: 'italic' }}>{prompt}</p>
      <QuizLink href={quizUrl} label={`take the ${quizLabel}`} />
    </div>
  )
}
...
function RitualwearContent({ data }) {
  ...
  if (!profile?.kibbe_type) return (
    <EmptyState
      prompt="Your style profile is empty. Take the Style Bible quiz to unlock your body type, colour season, and personal style words."
      quizUrl="https://wear.ritualware.app/quiz"
      quizLabel="Style Bible quiz"
    />
  )
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
        {profile.kibbe_type   && <Chip label="body type"     value={fmt(profile.kibbe_type)}   color="#C4717A" />}
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
            {profile.style_words.slice(0, 5).map(w => (
              <span key={w} style={{ ...mono, fontSize: '0.58rem', color: '#C4717A', background: 'rgba(196,113,122,0.08)', border: '1px solid rgba(196,113,122,0.2)', borderRadius: '3px', padding: '0.2rem 0.5rem', textTransform: 'capitalize' }}>{fmt(w)}</span>
            ))}
          </div>
        </div>
      )}
...
function GlowUpContent({ data }) {
  ...
  if (!glowUp && !styleFinder) return (
    <EmptyState
      prompt="No beauty data yet. Run your Glow Up audit to get your tier, then find your style archetype."
      quizUrl="https://glowup.ritualware.app/glow-up"
      quizLabel="Glow Up audit"
    />
  )
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
      {glowUp?.result?.overall_tier && <Chip label="glow tier"      value={fmt(glowUp.result.overall_tier)} color="#C4A96E" />}
      {styleFinder?.archetype       && <Chip label="style archetype" value={fmt(styleFinder.archetype)} />}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', marginTop: '0.15rem' }}>
        {!glowUp      && <QuizLink href="https://glowup.ritualware.app/glow-up"     label="run your glow up audit" />}
        {!styleFinder && <QuizLink href="https://glowup.ritualware.app/style-finder" label="find your style archetype" />}
      </div>
    </div>
  )
}
...
function RitualwhereContent({ data }) {
  ...
  if (!neighborhood) return (
    <EmptyState
      prompt="Where do you actually belong? Take the neighborhood quiz to find your NYC match."
      quizUrl="https://where.ritualware.app/neighborhood"
      quizLabel="neighborhood quiz"
    />
  )
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
      {neighborhood?.top_match   && <Chip label="your neighborhood" value={fmt(neighborhood.top_match)}  color="#6AAD8A" />}
      {burnout?.burnout_type     && <Chip label="burnout type"      value={fmt(burnout.burnout_type)} />}
      {burnout?.severity != null && <Chip label="severity"          value={`${burnout.severity} / 10`} />}
      {!burnout && <QuizLink href="https://where.ritualware.app/burnout" label="take the burnout audit" />}
    </div>
  )
}
...
function RitualwealthContent({ data }) {
  ...
  if (!firePlan) return (
    <EmptyState
      prompt="Your FIRE number is waiting. Take the quiz to map your path to financial freedom."
      quizUrl="https://wealth.ritualware.app/quiz/fire_type"
      quizLabel="FIRE quiz"
    />
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {firePlan.fire_type    && <Chip label="fire type"     value={fmt(firePlan.fire_type)} color="#A89BC4" />}
...
function AtelierContent({ data }) {
  ...
  if (!projects.length && !goals.length) return (
    <EmptyState
      prompt="Track your creative work here. Add a project or goal in m'atelier to get started."
      quizUrl="https://studio.ritualware.app"
      quizLabel="m'atelier"
    />
  )
